...
module.exports = {
    ...
    execute: async(client, message, args, data, db) => {
      ...
      if(args[0] == 'create') {
        ...
        if (!COINS || isNaN(COINS) || COINS < 1) return message.channel.send(`:sob: Commande incorrecte !\nPour acheter une carte-cadeau, faite \`-giftcode create <NombreDePièces> <Code>\``)
        if (COINS > data.coins) return message.channel.send(`:sob: J\'ai essayer de trouver votre solde dans ma base de données... Mais j\'ai remarquer que vous n\'aviez pas assès de pièces.\n*Essayez de rejoindre certains serveurs avec la commande -farm*`)
      ...
      .addField(`Nom du code cadeau :`, `${CODE}`, false)
      ...
      db.set(`logs_${message.author.id}`, data.logs)
        ...
        if (COINSTOADD == null || COINSTOADD == 0) {
              message.channel.send('Ce code a déjà été utilisé ou n\'est pas valide !')
                ...
                .setDescription(`<:cadeau:912801643597291520> Tu viens de récupérer \`${COINSTOADD}\` pièces !`)
          ...
          .setAuthor(client.user.username, client.user.displayAvatarURL())
          .setDescription(`\`-giftcode create <10> <MonCode>\` : Si vous voulez offrir une carte cadeau à un amis\n\`-giftcode redeem <MonCode>\` : Pour récupere un code cadeau`)
