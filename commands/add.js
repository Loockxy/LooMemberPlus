...
//
    ...
    .setTitle('LooMember+ | Logs (AddBal Command)')
    .setAuthor(client.user.username, client.user.displayAvatarURL())
    .setDescription(`**User: <@${message.author.id}>**, viens d'ajouter **${pay}** pièce(s) à <@${user.id}>`)
    .setColor('#2f3136')
    .setFooter(config.EmbedFooter, user.displayAvatarURL({ format: "png", dynamic: true }))
    let embedede = new Discord.MessageEmbed()
    .setAuthor(client.user.username, client.user.displayAvatarURL())
    .setDescription(`<:verif:912801644536807444> <@${user.id}> vous avez reçu \`${pay}\` pièce(s).`)
    .setColor('#2f3136')
